#!/usr/bin/env python3
"""
Huawei AppGallery Connect - Publishing Automation v6.0
======================================================

Complete Publishing Script with ALL App Data, Description, Category, Icons & APK
...
4. APK upload via OBS
5. Distribution countries (Arab countries)
6. Release notes update
7. Submit for review

Usage:
    python3 huawei_publish_v6.py              # Full publish workflow
    python3 huawei_publish_v6.py --check      # Check app status only
    python3 huawei_publish_v6.py --setup      # Set up app info + icon + description only
    python3 huawei_publish_v6.py --upload     # Upload APK only
    python3 huawei_publish_v6.py --submit     # Submit for review only
    python3 huawei_publish_v6.py --info       # Update app info + description + icon only
"""
...
class HuaweiPublisher:
    """Huawei AppGallery Connect Publisher"""
    ...
    def update_app_info(self):
        ...
        # Set app type to Games (parentType=13), Free app, Arab countries
        r = self._api('PUT', '/publish/v2/app-info', body={
            "parentType": PARENT_TYPE,  # 13 = Games
            "isFree": 1,  # Free app
            "distCountryCodes": ARABIC_COUNTRIES
        }, params={'appId': APP_ID})
        
        if r and r.get('ret', {}).get('code') == 0:
            self._log("[OK]", f"App info updated - Category: Games, Free, Countries: {', '.join(ARABIC_COUNTRIES)}")
        else:
            msg = r.get('ret', {}).get('msg', '?') if r else '?'
            self._log("[FAIL]", f"App info update failed: {msg}")
            if 'harmony category' in str(msg).lower():
                self._log("[WARN]", "HARMONY CATEGORY NOT SET - Please set it manually via web console!")
        return r
    
    # ========== UPDATE ARABIC DESCRIPTION ==========
    def update_arabic_description(self):
        """Update Arabic language info: name, description, brief, release notes"""
        ...
        if r and r.get('ret', {}).get('code') == 0:
            self._log("[OK]", "Arabic description updated successfully")
        else:
            msg = r.get('ret', {}).get('msg', '?') if r else '?'
    ...
    def update_english_description(self):
        """Update English language info: name, description, brief, release notes"""
        self._log("[EN]", "Updating English description...")
        
        body = {
            "lang": "en-US",
            "appName": APP_NAME_AR,  # Keep Arabic name as display name
        ...
        r = self._api('PUT', '/publish/v2/app-language-info', body=body, params={'appId': APP_ID})
        if r and r.get('ret', {}).get('code') == 0:
            self._log("[OK]", "English description updated successfully")
        else:
            msg = r.get('ret', {}).get('msg', '?') if r else '?'
    ...
    def upload_icon(self):
        ...
        if icon_512_oid or icon_216_oid:
            for lang in ['ar', 'en-US']:
                ...
                if r and r.get('ret', {}).get('code') == 0:
                    self._log("[OK]", f"Icons linked to app ({lang})")
                else:
                    msg = r.get('ret', {}).get('msg', '?') if r else '?'
    ...
    def upload_feature_graphic(self):
        """Upload feature graphic (1024x500) for store listing"""
    ...
    def upload_screenshots(self):
        """Upload 3 phone screenshots for app store listing"""
        screenshot_files = [
        ...
        if screenshot_oids:
            # Link screenshots as deviceMaterials
            for lang in ['ar', 'en-US']:
                materials = [
    ...
    # ========== UPLOAD APK ==========
    def upload_apk(self):
        """Download and upload APK via OBS"""
        # Download APK if not present or too small
        ...
        if not obs or obs.get('ret', {}).get('code') != 0:
            msg = obs.get('ret', {}).get('msg', '?') if obs else '?'
            self._log("[FAIL]", f"APK OBS URL failed: {msg}")
            return None, 0
        
        url_info = obs.get('urlInfo', {})
        object_id = url_info.get('objectId', '')
        
        # Step 2: Upload APK to OBS
...
After setting, re-run: python3 huawei_publish_v6.py
""")
    
    # ========== FULL PUBLISH WORKFLOW ==========
    def publish(self):
        """Full publishing workflow"""
        self._separator("Huawei AppGallery - Publishing v6.0")
        ...
        if sr and sr.get('ret', {}).get('code') == 0:
            ...
            print(f"  Category: Games (Trivia/Educational)")
            print(f"  Platform: Android + HarmonyOS")
            print(f"  Countries: {len(ARABIC_COUNTRIES)} Arab countries")
    ...
    def setup_only(self):
        ...
        self._separator("Huawei AppGallery - App Setup v6.0")
        
        if not self.get_token():
            return
        
        self._separator("Step 1: App Info")
        self.get_app_info()
        
        self._separator("Step 2: Harmony Category")
        if not self.check_harmony_category():
            return
        
        self._separator("Step 3: Arabic Description")
        self.update_arabic_description()
        
        self._separator("Step 4: English Description")
        self.update_english_description()
        
        self._separator("Step 5: Upload Icon")
        self.upload_icon()
        
        self._separator("Step 6: Upload Feature Graphic")
        self.upload_feature_graphic()
        
        self._separator("Step 7: Distribution Countries")
        self.set_countries()
        
        self._separator("Setup Complete!")
        print("  Run with --upload to upload APK")
        print("  Run with --submit to submit for review")
    ...
    def info_only(self):
        ...
        self._separator("Step 1: App Info & Category")
        self.update_app_info()
        
        self._separator("Step 2: Arabic Description")
        self.update_arabic_description()
        
        self._separator("Step 3: English Description")
        self.update_english_description()
        
        self._separator("Step 4: Upload Icon")
        self.upload_icon()
        
        self._separator("Step 5: Upload Feature Graphic")
        self.upload_feature_graphic()
    ...
    def check_status(self):
        """Check app status"""
        self._separator("Huawei AppGallery - App Status v6.0")
        print(f"  App: {APP_NAME_AR} ({APP_NAME_EN})")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Huawei AppGallery Publishing Tool v6.0')
